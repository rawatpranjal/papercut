...
Complete examples combining fetching, extraction, and AI-powered analysis.
...
.. code-block:: python
   ...
   from pathlib import Path
   from papercutter.intelligence import ReportGenerator
...
.. code-block:: python
   ...
   from pathlib import Path
   from papercutter.intelligence import StudyAid
...
Papercutter supports multiple LLM providers through LiteLLM.
...
.. code-block:: bash
   ...
   export PAPERCUTTER_MODEL=claude-3-opus-20240229

   # Use GPT-4
   export PAPERCUTTER_MODEL=gpt-4-turbo

   # Use a smaller/faster model
   export PAPERCUTTER_MODEL=claude-3-haiku-20240307

Configuration File
^^^^^^^^^^^^^^^^^^

Create ``~/.papercutter/config.yaml`` for persistent settings:
...
1. **Token limits**: Long papers may be truncated. Use ``focus`` parameter to target specific sections.
