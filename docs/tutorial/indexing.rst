...
Papercut can analyze document structure to detect sections, chapters, figures, and tables. This enables section-aware text extraction and intelligent navigation of long documents.
...
.. code-block:: bash
   ...
   papercut index textbook.pdf --type book

   # Save index to file
   papercut index paper.pdf -o index.json

   # Force re-indexing (ignore cache)
   papercut index paper.pdf --force
...
.. code-block:: bash
   ...
   papercut chapters textbook.pdf --json
...
.. code-block:: bash
   ...
   # Single page
   papercut read paper.pdf --pages 5

   # Page range
   papercut read paper.pdf --pages 10-14

   # Multiple ranges
   papercut read paper.pdf --pages 1-3,10-12,20
...
.. code-block:: bash
   ...
   papercut read paper.pdf --section "method"

   # Read by section ID (from index)
   papercut read paper.pdf --section 3
...
.. code-block:: bash
   ...
   # Read chapter 5
   papercut read textbook.pdf --chapter 5
...
4. **Combine with LLM features** - Use sections/chapters with ``papercut study`` for targeted study aids
